...
nowmd5=${nowmd5%% *}
local lastmd5=$(uci get AdGuard.AdGuard.gfwlistmd5 2>/dev/null)
if [ "$nowmd5" != "$lastmd5" ]; then
	uci set AdGuard.AdGuard.gfwlistmd5="$nowmd5"
	uci commit AdGuard
	[ "$1" == "noreload" ] || /etc/init.d/AdGuard reload
fi
}
configpath=$(uci get AdGuard.AdGuard.configpath 2>/dev/null)
[ "$1" == "del" ] && sed -i '/programaddstart/,/programaddend/d' $configpath && checkmd5 "$2" && exit 0
gfwupstream=$(uci get AdGuard.AdGuard.gfwupstream 2>/dev/null)
