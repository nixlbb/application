...
binpath=$(uci get AdGuard.AdGuard.binpath)
if [ -z "$binpath" ]; then
uci set AdGuard.AdGuard.binpath="/tmp/AdGuard/AdGuard"
binpath="/tmp/AdGuard/AdGuard"
fi
mkdir -p ${binpath%/*}
upxflag=$(uci get AdGuard.AdGuard.upxflag 2>/dev/null)

check_if_already_running(){
	running_tasks="$(ps |grep "AdGuard" |grep "update_core" |grep -v "grep" |awk '{print $1}' |wc -l)"
		...
		doupdate_core
			...
			if [ ! -z "$upxflag" ]; then
				...
				if [ $filesize -gt 8000000 ]; then
					...
					doupx
					mkdir -p "/tmp/AdGuardupdate/AdGuard" >/dev/null 2>&1
					rm -fr /tmp/AdGuardupdate/AdGuard/${binpath##*/}
					/tmp/upx-${upx_latest_ver}-${Arch}_linux/upx $upxflag $binpath -o /tmp/AdGuardupdate/AdGuard/${binpath##*/}
					rm -rf /tmp/upx-${upx_latest_ver}-${Arch}_linux
					/etc/init.d/AdGuard stop nobackup
					rm $binpath
					mv -f /tmp/AdGuardupdate/AdGuard/${binpath##*/} $binpath
...
doupdate_core(){
	...
	mkdir -p "/tmp/AdGuardupdate"
	rm -rf /tmp/AdGuardupdate/* >/dev/null 2>&1
	Archt="$(opkg info kernel | grep Architecture | awk -F "[ _]" '{print($2)}')"
	case $Archt in
	"i386")
	Arch="386"
	...
	*)
	...
	grep -v "^#" /usr/share/AdGuard/links.txt >/tmp/run/AdHlinks.txt
	while read link
	do
		eval link="$link"
		$downloader /tmp/AdGuardupdate/${link##*/} "$link" 2>&1
		if [ "$?" != "0" ]; then
			echo "download failed try another download"
			rm -f /tmp/AdGuardupdate/${link##*/}
		else
			local success="1"
			break
		fi 
	...
	if [ "${link##*.}" == "gz" ]; then
		tar -zxf "/tmp/AdGuardupdate/${link##*/}" -C "/tmp/AdGuardupdate/"
		if [ ! -e "/tmp/AdGuardupdate/AdGuard" ]; then
			echo -e "Failed to download core." 
			rm -rf "/tmp/AdGuardupdate" >/dev/null 2>&1
			EXIT 1
		fi
		downloadbin="/tmp/AdGuardupdate/AdGuard/AdGuard"
	else
		downloadbin="/tmp/AdGuardupdate/${link##*/}"
	fi
	...
	if [ "$?" == "1" ]; then
		echo "mv failed maybe not enough space please use upx or change bin to /tmp/AdGuard" 
		EXIT 1
	fi
	/etc/init.d/AdGuard start
	rm -rf "/tmp/AdGuardupdate" >/dev/null 2>&1
